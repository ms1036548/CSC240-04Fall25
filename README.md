...
# CSC240 – Phase 3: Static Site Generator & UI

This phase builds a **static website** that consumes the existing Phase 2 APIs (Data, Class, UI) **through Apache APISIX** and then serves the generated HTML with **Apache HTTP Server (httpd)**.

The workflow:

1. Start the three Java APIs (Data, Class, UI).
2. Make sure APISIX is running and routing requests to those APIs.
3. Run `SiteGenerator` to create HTML pages in `target/site/`.
4. Start Apache HTTPD and open `http://localhost/` in a browser to view the site.

---

## 1. Run the SiteGenerator

From the project root `CSC240-04Fall25`, open new terminal and run


```powershell
javac -cp "lib/*" src/site/SiteGenerator.java -d out
java -cp "out;lib/*" site.SiteGenerator

```
What this does:

Calls the UI API through APISIX at http://localhost:9080:

This will:

✔ Call your UI API through APISIX (http://localhost:9080)
✔ Generate HTML files into:
...
2. Start Apache HTTP Server (httpd)
...
If it shows no errors → server is live.

3. View the Final Website

Open a browser and go to:

http://localhost/

This will load the Global Education Dashboard (index.html) generated by SiteGenerator and served by Apache HTTPD.
You can then navigate to the other pages (e.g., countries.html, universities.html) using the links on the page.

Should show along the lines of :

<img width="1918" height="975" alt="image" src="https://github.com/user-attachments/assets/b1d5e5e4-792e-4d2c-9a92-a37f7b433d61" />

<img width="1918" height="962" alt="image" src="https://github.com/user-attachments/assets/1d8144f2-9426-44c7-83a8-cef9f0c852be" />
